...
type TableErrorDetail struct {
	...
	Mode             string     `gorm:"not null;comment:'运行模式'" json:"mode"`
	Status           string     `gorm:"not null;comment:'运行状态'" json:"status"`
...
func (e *Engine) GetTableErrorDetailCount(schemaName string) (int64, error) {
	var totals int64
	if err := e.GormDB.Model(&TableErrorDetail{}).
		Where(`source_schema_name = ?`, strings.ToUpper(schemaName)).
