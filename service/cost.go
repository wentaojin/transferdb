...
WHERE
	...
	AND s.segment_type = 'LOBINDEX'`, owner))
		...
		_, tableRows, err := Query(e.OracleDB, fmt.Sprintf(`select SUM(num_rows) NUM_ROWS from dba_tables where OWNER IN (%s)`, owner))
		if err != nil {
			return vals, err
		}
...
func (e *Engine) GetOracleCodeObject(schemaName []string) ([]map[string]string, error) {
	var sql string
	if len(schemaName) == 0 {
		sql = `select OWNER,NAME,TYPE,SUM(LINE) LINES from DBA_SOURCE where owner not in  ('HR','DVF','DVSYS','LBACSYS','MDDATA','OLAPSYS','ORDPLUGINS','ORDDATA','MDSYS','SI_INFORMTN_SCHEMA','ORDSYS','CTXSYS','OJVMSYS','WMSYS','ANONYMOUS','XDB','GGSYS','GSMCATUSER','APPQOSSYS','DBSNMP','SYS$UMF','ORACLE_OCM','DBSFWUSER','REMOTE_SCHEDULER_AGENT','XS$NULL','DIP','GSMROOTUSER','GSMADMIN_INTERNAL','GSMUSER','OUTLN','SYSBACKUP','SYSDG','SYSTEM','SYSRAC','AUDSYS','SYSKM','SYS','OGG','SPA','APEX_050000','SQL_MONITOR') GROUP BY OWNER,NAME,TYPE ORDER BY LINES DESC`
	} else {
		sql = fmt.Sprintf(`select OWNER,NAME,TYPE,SUM(LINE) LINES from DBA_SOURCE where OWNER IN (%s) GROUP BY OWNER,NAME,TYPE ORDER BY LINES DESC`, strings.Join(schemaName, ","))
	}
