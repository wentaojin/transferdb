...
func (e *Engine) GetOracleSynonymObjectType(schemaName []string) ([]map[string]string, error) {
	...
	if len(schemaName) == 0 {
		...
		sql = fmt.Sprintf(`select OWNER,count(*) COUNT from dba_synonyms where TABLE_OWNER = '%s' group by OWNER ORDER BY COUNT DESC`, strings.Join(schemaName, ","))
	}
...
func (e *Engine) GetOracleCodeObject(schemaName []string) ([]map[string]string, error) {
	var sql string
	if len(schemaName) == 0 {
		sql = `select OWNER,NAME,TYPE,SUM(LINE) ROWS from ALL_SOURCE where owner not in  ('HR','DVF','DVSYS','LBACSYS','MDDATA','OLAPSYS','ORDPLUGINS','ORDDATA','MDSYS','SI_INFORMTN_SCHEMA','ORDSYS','CTXSYS','OJVMSYS','WMSYS','ANONYMOUS','XDB','GGSYS','GSMCATUSER','APPQOSSYS','DBSNMP','SYS$UMF','ORACLE_OCM','DBSFWUSER','REMOTE_SCHEDULER_AGENT','XS$NULL','DIP','GSMROOTUSER','GSMADMIN_INTERNAL','GSMUSER','OUTLN','SYSBACKUP','SYSDG','SYSTEM','SYSRAC','AUDSYS','SYSKM','SYS','OGG','SPA','APEX_050000','SQL_MONITOR') GROUP BY OWNER,NAME,TYPE ORDER BY ROWS DESC`
	} else {
		sql = fmt.Sprintf(`select OWNER,NAME,TYPE,SUM(LINE) ROWS from ALL_SOURCE where OWNER IN (%s) GROUP BY OWNER,NAME,TYPE ORDER BY ROWS DESC`, strings.Join(schemaName, ","))
	}
...
func (e *Engine) GetOracleTableIndexLengthOver3072(schemaName []string) ([]map[string]string, error) {
	var sql string
	if len(schemaName) == 0 {
		sql = `select INDEX_OWNER,INDEX_NAME,TABLE_NAME,sum(COLUMN_LENGTH) COUNT  from dba_ind_columns where TABLE_OWNER not in ('HR','DVF','DVSYS','LBACSYS','MDDATA','OLAPSYS','ORDPLUGINS','ORDDATA','MDSYS','SI_INFORMTN_SCHEMA','ORDSYS','CTXSYS','OJVMSYS','WMSYS','ANONYMOUS','XDB','GGSYS','GSMCATUSER','APPQOSSYS','DBSNMP','SYS$UMF','ORACLE_OCM','DBSFWUSER','REMOTE_SCHEDULER_AGENT','XS$NULL','DIP','GSMROOTUSER','GSMADMIN_INTERNAL','GSMUSER','OUTLN','SYSBACKUP','SYSDG','SYSTEM','SYSRAC','AUDSYS','SYSKM','SYS') group by INDEX_OWNER,INDEX_NAME,TABLE_NAME  having COUNT  > 3072`
	} else {
		sql = fmt.Sprintf(`select INDEX_OWNER,INDEX_NAME,TABLE_NAME,sum(COLUMN_LENGTH) COUNT from dba_ind_columns where TABLE_OWNER = '%s' group by INDEX_OWNER,INDEX_NAME,TABLE_NAME  having COUNT > 3072`, strings.Join(schemaName, ","))
	}
...
func (e *Engine) GetOracleTableColumnCountsOver512(schemaName []string) ([]map[string]string, error) {
	var sql string
	if len(schemaName) == 0 {
		sql = `SELECT OWNER,TABLE_NAME,COUNT(COLUMN_NAME) COUNT FROM DBA_TAB_COLUMNS  WHERE  OWNER NOT IN ('HR','DVF','DVSYS','LBACSYS','MDDATA','OLAPSYS','ORDPLUGINS','ORDDATA','MDSYS','SI_INFORMTN_SCHEMA','ORDSYS','CTXSYS','OJVMSYS','WMSYS','ANONYMOUS','XDB','GGSYS','GSMCATUSER','APPQOSSYS','DBSNMP','SYS$UMF','ORACLE_OCM','DBSFWUSER','REMOTE_SCHEDULER_AGENT','XS$NULL','DIP','GSMROOTUSER','GSMADMIN_INTERNAL','GSMUSER','OUTLN','SYSBACKUP','SYSDG','SYSTEM','SYSRAC','AUDSYS','SYSKM','SYS','OGG','SPA','APEX_050000','SQL_MONITOR') GROUP BY  OWNER,TABLE_NAME HAVING COUNT > 512 ORDER BY OWNER,TABLE_NAME`
	} else {
		sql = fmt.Sprintf(`SELECT OWNER,TABLE_NAME,COUNT(COLUMN_NAME) COUNT FROM DBA_TAB_COLUMNS  WHERE  OWNER IN (%s) GROUP BY  OWNER,TABLE_NAME HAVING COUNT > 512 ORDER BY OWNER,TABLE_NAME`, strings.Join(schemaName, ","))
	}
...
func (e *Engine) GetOracleTableIndexCountsOver64(schemaName []string) ([]map[string]string, error) {
	var sql string
	if len(schemaName) == 0 {
		sql = `select TABLE_OWNER,TABLE_NAME,COUNT(INDEX_NAME) COUNT FROM dba_ind_columns WHERE TABLE_OWNER NOT IN ('HR','DVF','DVSYS','LBACSYS','MDDATA','OLAPSYS','ORDPLUGINS','ORDDATA','MDSYS','SI_INFORMTN_SCHEMA','ORDSYS','CTXSYS','OJVMSYS','WMSYS','ANONYMOUS','XDB','GGSYS','GSMCATUSER','APPQOSSYS','DBSNMP','SYS$UMF','ORACLE_OCM','DBSFWUSER','REMOTE_SCHEDULER_AGENT','XS$NULL','DIP','GSMROOTUSER','GSMADMIN_INTERNAL','GSMUSER','OUTLN','SYSBACKUP','SYSDG','SYSTEM','SYSRAC','AUDSYS','SYSKM','SYS','OGG','SPA','APEX_050000','SQL_MONITOR') GROUP BY TABLE_OWNER,TABLE_NAME HAVING COUNT >64 ORDER BY TABLE_OWNER,TABLE_NAME`
	} else {
		sql = fmt.Sprintf(`select TABLE_OWNER,TABLE_NAME,COUNT(INDEX_NAME) COUNT FROM dba_ind_columns WHERE TABLE_OWNER = '%s' GROUP BY TABLE_OWNER,TABLE_NAME HAVING COUNT >64 ORDER BY TABLE_OWNER,TABLE_NAME`, strings.Join(schemaName, ","))
	}
...
func (e *Engine) GetOracleUsernameLength(schemaName []string) ([]map[string]string, error) {
	...
	if len(schemaName) == 0 {
		...
		sql = fmt.Sprintf(`select USERNAME,ACCOUNT_STATUS,CREATED from dba_users where username = '%s' AND length(USERNAME)>64`, strings.Join(schemaName, ","))
	}
...
func (e *Engine) GetOracleTableIndexNameLength(schemaName []string) ([]map[string]string, error) {
	...
	if len(schemaName) == 0 {
		...
		sql = fmt.Sprintf(`SELECT INDEX_OWNER,TABLE_NAME,INDEX_NAME FROM DBA_IND_COLUMNS WHERE TABLE_OWNER = '%s' AND LENGTH(COLUMN_NAME) > 64 ORDER BY INDEX_OWNER,TABLE_NAME,INDEX_NAME`, strings.Join(schemaName, ","))
	}
...
func (e *Engine) GetOracleTableSequenceNameLength(schemaName []string) ([]map[string]string, error) {
	...
	if len(schemaName) == 0 {
		...
		sql = fmt.Sprintf(`SELECT SEQUENCE_OWNER,SEQUENCE_NAME,ORDER_FLAG FROM DBA_SEQUENCES WHERE SEQUENCE_OWNER = '%s' AND LENGTH(SEQUENCE_NAME)>64 ORDER BY SEQUENCE_OWNER,SEQUENCE_NAME,ORDER_FLAG`, strings.Join(schemaName, ","))
	}
