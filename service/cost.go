...
func (e *Engine) GetOracleTableIndexLengthOver3072(schemaName []string) ([]map[string]string, error) {
	...
	if len(schemaName) == 0 {
		...
		sql = fmt.Sprintf(`select INDEX_OWNER,INDEX_NAME,TABLE_NAME,sum(COLUMN_LENGTH) COUNT from dba_ind_columns where TABLE_OWNER IN (%s) group by INDEX_OWNER,INDEX_NAME,TABLE_NAME  having COUNT > 3072`, strings.Join(schemaName, ","))
	}
...
func (e *Engine) GetOracleTableIndexCountsOver64(schemaName []string) ([]map[string]string, error) {
	...
	if len(schemaName) == 0 {
		...
		sql = fmt.Sprintf(`select TABLE_OWNER,TABLE_NAME,COUNT(INDEX_NAME) COUNT FROM dba_ind_columns WHERE TABLE_OWNER IN (%s) GROUP BY TABLE_OWNER,TABLE_NAME HAVING COUNT >64 ORDER BY TABLE_OWNER,TABLE_NAME`, strings.Join(schemaName, ","))
	}
