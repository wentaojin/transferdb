...
type DataDiffMeta struct {
	...
	SourceSchemaName string `gorm:"not null;index:idx_schema_table_where;comment:'源端 schema'" json:"source_schema_name"`
	SourceTableName  string `gorm:"not null;index:idx_schema_table_where;comment:'源端表名'" json:"source_table_name"`
	...
	Where            string `gorm:"not null;index:idx_schema_table_where;comment:'查询 where 条件'" json:"where"`
...
func (e *Engine) DeleteDataDiffMeta(schemaName, tableName, whereS string) error {
	if whereS == "1 = 1" {
		if err := e.GormDB.Model(DataDiffMeta{}).Where("1 = 1 AND source_schema_name = ? AND source_table_name = ?",
			strings.ToUpper(schemaName),
			strings.ToUpper(tableName)).Delete(&DataDiffMeta{}).Error; err != nil {
			return fmt.Errorf("get custom data diff meta failed: %v", err)
		}
	} else {
		if err := e.GormDB.Model(DataDiffMeta{}).Where("source_schema_name = ? AND source_table_name = ? AND where = ?",
			strings.ToUpper(schemaName),
			strings.ToUpper(tableName),
			whereS).Delete(&DataDiffMeta{}).Error; err != nil {
			return fmt.Errorf("get custom data diff meta failed: %v", err)
		}
	}
