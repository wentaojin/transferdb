...
// CSV 数据处理
func (e *Engine) CSVRowProcessor(querySQL string, csvConfig CSVConfig) ([]string, [][]string, error) {
	...
	cols, err := rows.Columns()
	if err != nil {
		return cols, rowsResult, fmt.Errorf("[%v] error on general query rows.Columns failed", err.Error())
	}
		...
		for _, raw := range rawResult {
			...
			if raw == nil {
				...
				if ok {
					...
					if strings.ToUpper(csvConfig.Charset) == utils.OracleGBKCharacterSet {
					...
					if csvConfig.Delimiter == "" {
						...
						results = append(results, utils.StringsBuilder(csvConfig.Delimiter, bs, csvConfig.Delimiter))
					}
				}
			}
		}
		rowsResult = append(rowsResult, results)
	}
