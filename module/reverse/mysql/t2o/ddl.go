...
func (d *DDL) WriteFile(w *reverse.Write) (string, error) {
	...
	// 表 with 主键
	sqlRev.WriteString("/*\n")
	sqlRev.WriteString(" tidb table reverse sql \n")

	sw := table.NewWriter()
	sw.SetStyle(table.StyleLight)
	sw.AppendHeader(table.Row{"#", "TABLE TYPE", "TIDB", "ORACLE", "SUGGEST"})
	...
	if len(compDDLS) > 0 {
		sqlComp.WriteString("/*\n")
		sqlComp.WriteString(" tidb table structure object maybe oracle has compatibility, skip\n")
		tw := table.NewWriter()
		tw.SetStyle(table.StyleLight)
		tw.AppendHeader(table.Row{"#", "TIDB", "ORACLE", "SUGGEST"})
...
func (d *DDL) GenDDLStructure() ([]string, []string) {
	...
	zap.L().Info("reverse tidb table structure",
