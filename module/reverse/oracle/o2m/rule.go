...
func (r *Rule) GenTableSuffix() (string, error) {
	...
	tableCharset = common.MigrateTableStructureDatabaseCharsetMap[common.TaskTypeOracle2MySQL][r.SourceDBCharset]

	// schema、db、table collation
	if r.OracleCollation {
		// table collation
		if r.SourceTableCollation != "" {
			if val, ok := common.MigrateTableStructureDatabaseCollationMap[common.TaskTypeOracle2MySQL][r.SourceTableCollation][tableCharset]; ok {
				tableCollation = val
			} else {
		...
		if r.SourceTableCollation == "" && r.SourceSchemaCollation != "" {
			if val, ok := common.MigrateTableStructureDatabaseCollationMap[common.TaskTypeOracle2MySQL][r.SourceSchemaCollation][tableCharset]; ok {
				tableCollation = val
			} else {
		...
		// db collation
		if val, ok := common.MigrateTableStructureDatabaseCollationMap[common.TaskTypeOracle2MySQL][r.SourceDBNLSComp][tableCharset]; ok {
			tableCollation = val
		} else {
...
func (r *Rule) GenTableComment() (tableComment string, err error) {
	if len(r.TableColumnINFO) > 0 && r.TableCommentINFO[0]["COMMENTS"] != "" {
		convertUtf8Raw, err := common.CharsetConvert([]byte(r.TableCommentINFO[0]["COMMENTS"]), common.MigrateTableStructureDatabaseCharsetMap[common.TaskTypeOracle2MySQL][common.StringUPPER(r.SourceDBCharset)], common.MYSQLCharsetUTF8MB4)
		if err != nil {
...
func (r *Rule) GenTableColumn() (tableColumns []string, err error) {
	for _, rowCol := range r.TableColumnINFO {
		...
		if !strings.EqualFold(rowCol["COMMENTS"], "") {
			convertUtf8Raw, err := common.CharsetConvert([]byte(rowCol["COMMENTS"]), common.MigrateTableStructureDatabaseCharsetMap[common.TaskTypeOracle2MySQL][common.StringUPPER(r.SourceDBCharset)], common.MYSQLCharsetUTF8MB4)
			if err != nil {
		...
		if val, ok := r.TableColumnDefaultValRule[rowCol["COLUMN_NAME"]]; ok {
			convertUtf8Raw, err := common.CharsetConvert([]byte(val), common.MigrateTableStructureDatabaseCharsetMap[common.TaskTypeOracle2MySQL][common.StringUPPER(r.SourceDBCharset)], common.MYSQLCharsetUTF8MB4)
			if err != nil {
