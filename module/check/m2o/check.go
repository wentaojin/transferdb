...
func (r *Check) Check() error {
	...
	tablesByCfg, _, err := filterCFGTable(r.cfg, r.mysql)
	if err != nil {
		return err
	}
	...
	// 任务检查表
	tasks := GenCheckTaskTable(r.cfg.MySQLConfig.DBType, r.cfg.MySQLConfig.SchemaName, r.cfg.OracleConfig.SchemaName, oracleDBCharacterSet, nlsSort, nlsComp, oracleTableCollation, oracleSchemaCollation, oracleDBCollation,
	...
	for _, task := range tasks {
		...
		g.Go(func() error {
			...
			err = NewChecker(r.ctx, oracleTableInfo, mysqlTableInfo,
				r.cfg.DBTypeS, r.cfg.DBTypeT, mysqlDBVersion, r.cfg.MySQLConfig.DBType, oracleDBVersion, oracleDBExtendMode, r.metaDB).Writer(f)
			if err != nil {
