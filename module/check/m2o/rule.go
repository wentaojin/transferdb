...
func MySQLTableColumnMapRuleCheck(
	...
	if strings.ToUpper(common.MySQLDBCharacterSetMap[mysqlColInfo.CharacterSet]) == "GBK" {
		oracleCharacterSet = strings.ToLower(common.MySQLDBCharacterSetMap[mysqlColInfo.CharacterSet])
	} else {
		oracleCharacterSet = strings.ToLower(common.MySQLDBCharacterSetMap[mysqlColInfo.CharacterSet])
	}
	oracleCollation = strings.ToLower(common.MySQLDBCollationMap[mysqlColInfo.Collation])
