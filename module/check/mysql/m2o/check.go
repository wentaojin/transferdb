...
func (r *Check) Check() error {
	...
	sourceTableNameRules, err := meta.NewTableNameRuleModel(r.metaDB).DetailTableNameRule(r.ctx, &meta.TableNameRule{
		...
		SchemaNameS: r.cfg.MySQLConfig.SchemaName,
		SchemaNameT: r.cfg.OracleConfig.SchemaName,
	})
	if err != nil {
		return err
	}
	...
	// 判断下游数据库是否存在 mysql 表
	mysqlTables, err := r.oracle.GetOracleSchemaTable(r.cfg.OracleConfig.SchemaName)
	if err != nil {
		return err
	}
	...
	if !ok {
		return fmt.Errorf("mysql tables %v isn't exist in the oracle schema [%v], please create", noExistTables, r.cfg.OracleConfig.SchemaName)
	}
	...
	oracleDBCharset := strings.Split(oracleDBCharacterSet, ".")[1]
	if _, ok := common.MigrateTableStructureDatabaseCharsetMap[common.TaskTypeOracle2MySQL][oracleDBCharset]; !ok {
	...
	if _, ok := common.MigrateTableStructureDatabaseCollationMap[common.TaskTypeOracle2MySQL][strings.ToUpper(nlsSort)]; !ok {
		return fmt.Errorf("oracle db nls sort [%s] isn't support", nlsSort)
	}
	if _, ok := common.MigrateTableStructureDatabaseCollationMap[common.TaskTypeOracle2MySQL][strings.ToUpper(nlsComp)]; !ok {
	...
	if common.VersionOrdinal(oracleDBVersion) >= common.VersionOrdinal(common.OracleTableColumnCollationDBVersion) {
		...
		oracleDBExtendMode, err = r.oracle.GetOracleExtendedMode()
		if err != nil {
			return err
		}
	}
	...
	// 任务检查表
	tasks := GenCheckTaskTable(r.cfg.MySQLConfig.SchemaName, r.cfg.OracleConfig.SchemaName, oracleDBCharacterSet, nlsSort, nlsComp, oracleTableCollation, oracleSchemaCollation, oracleDBCollation,
	...
	checkFile := filepath.Join(r.cfg.CheckConfig.CheckSQLDir, fmt.Sprintf("check_%s.sql", r.cfg.MySQLConfig.SchemaName))
	...
	for _, task := range tasks {
		...
		g.Go(func() error {
			...
			err = NewChecker(r.ctx, oracleTableInfo, mysqlTableInfo,
				r.cfg.DBTypeS, r.cfg.DBTypeT, mysqlDBVersion, oracleDBVersion, oracleDBExtendMode, r.metaDB).Writer(f)
			if err != nil {
	...
	zap.L().Info("check", zap.String("output", filepath.Join(r.cfg.CheckConfig.CheckSQLDir, fmt.Sprintf("check_%s.sql", r.cfg.MySQLConfig.SchemaName))))
	if len(failedTotals) == 0 {
		zap.L().Info("check table mysql to oracle finished",
			zap.Int("table totals", len(waitSyncMetas)),
			zap.Int("table success", len(succTotals)),
			zap.Int("table failed", 0),
			zap.String("cost", time.Now().Sub(startTime).String()))
	} else {
		zap.L().Warn("check table mysql to oracle finished",
			zap.Int("table totals", len(waitSyncMetas)),
			zap.Int("table success", len(succTotals)),
